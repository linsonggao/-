...
use core\lib\db\exception\DataNotFoundException;
use core\lib\db\exception\ModelNotFoundException;
...
use core\lib\model\relation\OneToOne;
...
class Query
{
    ...
    public function getTable($name = '')
    {
        if ($name || empty($this->table)) {
            ...
            if ($name) {
                $tableName .= Utils::parseName($name);
            }
        } else {
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\core\lib\\paginator\\driver\\' . ucwords($config['type']);
    ...
    public function update(array $data = [])
    {
        ...
        if (empty($options['where'])) {
            ...
            if (is_string($pk) && isset($data[$pk])) {
                $where[$pk] = $data[$pk];
                if (!isset($key)) {
                    $key = 'core\lib:' . $options['table'] . '|' . $data[$pk];
                }
                unset($data[$pk]);
            } elseif (is_array($pk)) {
                // 增加复合主键支持
    ...
    protected function getCacheKey($value, $options, $bind = [])
    {
        ...
        if (isset($data)) {
            return 'core\lib:' . (is_array($options['table']) ? key($options['table']) : $options['table']) . '|' . $data;
        } else {
    ...
    public function find($data = null)
    {
        ...
        if (empty($options['fetch_sql']) && !empty($options['cache'])) {
            ...
            if (true === $cache['key'] && !is_null($data) && !is_array($data)) {
                $key = 'core\lib:' . (is_array($options['table']) ? key($options['table']) : $options['table']) . '|' . $data;
    ...
    public function delete($data = null)
    {
        ...
        if (!is_null($data) && true !== $data) {
            if (!isset($key) && !is_array($data)) {
                // 缓存标识
                $key = 'core\lib:' . $options['table'] . '|' . $data;
            }
            // AR模式分析主键条件
            $this->parsePkWhere($data, $options);
        } elseif (!isset($key) && is_string($pk) && isset($options['where']['AND'][$pk])) {
