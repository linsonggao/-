...
class Administrator extends AdminAuth
{
    ...
    {
        $list =  User::where('status','>=','0')->where('is_delete',0)->paginate();
    ...
    public function add()
    {
        ...
        $rule = [
			...
			'username|用户名' => 'require|alphaDash|min:5|unique:user',
    ...
    public function update($id)
    {
        ...
        $rule = [
            //管理员登陆字段验证
            'nickname|昵称'   => 'require|min:2',
			'username|用户名' => 'require|alphaDash|min:5|unique:system_user,username,'.$id.',system_uid', //更新时用户名唯一，但排除当前ID, 
			'mobile|手机号'   => 'length:11',
        ];
        $msg = [];
